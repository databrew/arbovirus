...
new_cols <- matrix( unlist( strsplit(tallies$Testing, "\\]\\[") ), ncol=2, byrow=T)
tallies <- tallies %>% 
  mutate(Arbovirus = new_cols[,1], Testing = new_cols[,2])
tallies$Response <- as.numeric( tallies$Response)
tallies <- tallies %>% 
  group_by(Arbovirus, Testing) %>% 
  summarize(Total = sum(Response, na.rm=T))
tallies$Arbovirus <- factor( tallies$Arbovirus )
tallies$Testing <- factor( tallies$Testing )
mat <- matrix( data=tallies$Total, ncol=length(levels(tallies$Arbovirus)), byrow=F)
colnames( mat ) <- levels( tallies$Arbovirus )
rownames( mat ) <- levels( tallies$Testing )
knitr::kable( mat )
# knitr::kable( table( tallies$Arbovirus, tallies$Testing, useNA="ifany") )
