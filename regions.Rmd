...
if( file.exists("data/data.RData") ){
  ...
  # add _serology_ and _direct detection_ variables to simplify summary of question 17:
  ...
  serology <- cbind( q17[,2] | q17[,3] | q17[,4],     # Chikungunya 
                     q17[,9] | q17[,10] | q17[,11],   # Dengue 
                     q17[,16] | q17[,17] | q17[,18],  # Yellow fever 
                     q17[,23] | q17[,24] | q17[,25],  # Zika
                     q17[,30] | q17[,31] | q17[,32] ) # other 
  for(i in 1:ncol(serology)){
    serology[,i] = ifelse( serology[,i], "Yes", "No")
  }
  colnames( serology ) <- c( "Chikungunya][Serology",
                             "Dengue][Serology",
                             "Yellow fever][Serology",
                             "Zika][Serology",
                             "Other][Serology")
  data <- cbind( data, serology )
...
var_names <- c( "Chikungunya][Serology",
                "Dengue][Serology",
                "Yellow fever][Serology",
                "Zika][Serology",
                "Other][Serology")
serology <- data %>% select( all_of(var_names) )
colnames(serology) <- gsub("][Serology","",colnames(serology), fixed=TRUE)

df <- cbind(Region = data$Region, serology) %>% 
  pivot_longer(!Region, names_to="Arbovirus", values_to="Response") %>% 
...
colnames(direct) <- gsub("][Direct","",colnames(serology), fixed=TRUE)
