...
```{r}
regulartable( data.frame( Country=data$SI01, Contact=data$SI02) ) %>%
...
```{r}
regulartable( data.frame( Country=data$SI01, Title=data$SI03) ) %>%
  hline( i = 1:length(data$SI01)) %>%
  autofit()
```
...
```{r}
regulartable( data.frame( Country=data$SI01, 
                          Date=paste( day(data$SI04), "/", 
                                      month(data$SI04), "/",
                                      year(data$SI04)) ) ) %>%
  hline( i = 1:length(data$SI01)) %>%
  autofit()
...
#### 5. Which other arboviruses have circulated in your country at any time since the year 2000?  
...
#### 6b. For which other arboviruses do you have written surveillance and control plans for your country?
...
```{r}
data.frame(Country = data$SI01, Number = data$SII06c__filecount) %>%
  drop_na(Number) %>%
  filter(Number > 0 ) %>%
...
df <- cbind(Country = data$SI01, responses) %>% 
  mutate( Basic = factor( replace_na( as.character(Basic), "Not selected")),
          Repeat_basic = factor( replace_na( as.character(Repeat_basic), "Not selected")),
          Advanced = factor( replace_na( as.character(Advanced), "Not selected")),
          Repeat_advanced = factor( replace_na( as.character(Repeat_advanced), "Not selected")),
          None = factor( replace_na( as.character(None), "Not selected"))) 
...
```{r}
data.frame(Country = data$SI01, Number = data$SII11d__filecount) %>%
  drop_na(Number) %>%
  filter(Number > 0 ) %>%
  rename("Number of files"=Number) %>%
  regulartable() %>%
...
```{r}
data.frame(Country = data$SI01, Number = data$SII12d__filecount) %>%
  drop_na(Number) %>%
  filter(Number > 0 ) %>%
  rename("Number of files"=Number) %>%
  regulartable() %>%
...
colnames(responses)=c("Non","Outbreak")
responses <- cbind(Country = data$SI01, responses) %>% 
  mutate( Outbreak = factor( replace_na( as.character(Outbreak), "Unanswered"))) %>%
  mutate( Non = factor( replace_na( as.character(Non), "Unanswered")))
colnames(responses) <- c("Country",
                         "Non-outbreak (routine) annual percentage", 
                         "During outbreaks percentage per identifed cluster")
responses %>%
  regulartable() %>% 
  hline( part="all" ) %>%
  autofit()
...
```{r}
data.frame(Country = data$SI01, Number = data$SIV21b__filecount) %>%
  drop_na(Number) %>%
  filter(Number > 0 ) %>%
  rename("Number of files"=Number) %>%
  regulartable() %>%
...
cbind(Country=data$SI01, responses) %>%
  mutate( Response = factor( replace_na( as.character(Response), "Unanswered"))) %>%
  mutate( Response = recode_factor(.x = Response,
                                   "Yes, specific training is provided. If so, please specify:"="Yes, specific training is provided",
                                   "No"="No",
                                   "Unanswered"="Unanswered")) %>%
  regulartable() %>%
  hline( part="all") %>%
...
cbind(Country=data$SI01, responses) %>%
  mutate( Response = factor( replace_na( as.character(Response), "Unanswered"))) %>%
  mutate( Response = recode_factor(.x = Response,
                                   "Yes. If so, please specify in the comment field."="Yes",
                                   "No"="No",
                                   "Unanswered"="Unanswered")) %>%
  regulartable() %>%
  hline( part="all") %>%
...
```{r}
data.frame(Country = data$SI01, Number = data$SVI41d__filecount) %>%
  drop_na(Number) %>%
  filter(Number > 0 ) %>%
  rename("Number of files"=Number) %>%
  regulartable() %>%
...
```{r}
data.frame(Country = data$SI01, Number = data$SVI42c__filecount) %>%
  drop_na(Number) %>%
  filter(Number > 0 ) %>%
  rename("Number of files"=Number) %>%
  regulartable() %>%
...
```{r}
data.frame(Country = data$SI01, Number = data$SVIII48b__filecount) %>%
  drop_na(Number) %>%
  filter(Number > 0 ) %>%
  rename("Number of files"=Number) %>%
  regulartable() %>%
...
cbind(Country=data$SI01, responses) %>%
  mutate( Response = factor( replace_na( as.character(Response), "Unanswered"))) %>%
  mutate( Response = recode_factor(.x = Response,
                                   "Yes. If so, please specify institutions/agencies in the comments field:"="Yes",
                                   "No"="No",
                                   "Unanswered"="Unanswered")) %>%
  regulartable() %>%
  hline( part="all") %>%
