---
title: ' '
pagetitle: 'SEARO Q29 global'
output:
  html_document:
    includes:
      in_header: SEARO_Q29_global_header.html
params:
  region: SEARO
---
...
responses <- responses %>% 
  ...
  rename("Adulticiding"="Adulticiding (insecticide application against adult mosquitoes)",
         "Regulators" = "Insect growth regulators (eg , pyriproxyfen)",
         "Wolbachia" = "Wolbachia method",
         "Sterile" = "Sterile insect release")

responses %>% 
  group_by(Region) %>%
  summarize( Adulticiding = length( which( Adulticiding == "Yes")),
             Larviciding = length( which( Larviciding == "Yes")),
             Regulators = length( which( Regulators == "Yes")),
             Wolbachia = length( which( Wolbachia == "Yes")),
             Sterile = length( which( Sterile == "Yes")),
             None = length( which( None == "Yes")) ) %>%
  left_join( nums, by="Region") %>%
  mutate( Adulticiding = paste0( Adulticiding, " (", round(100*Adulticiding/n_countries), "%)"),
          Larviciding = paste0( Larviciding, " (", round(100*Larviciding/n_countries), "%)"),
          Regulators = paste0( Regulators, " (", round(100*Regulators/n_countries), "%)"),
          Wolbachia = paste0( Wolbachia, " (", round(100*Wolbachia/n_countries), "%)"),
          Sterile = paste0( Sterile, " (", round(100*Sterile/n_countries), "%)"),
          None = paste0( None, " (", round(100*None/n_countries), "%)") ) %>%
  select(-n_countries) %>%
  rename("Insect growth regulators" = "Regulators",
         "Wolbachia method" = "Wolbachia",
         "Sterile insect release" = "Sterile") %>% 
  regulartable %>% 
  theme_zebra(odd_header="#048ECA", odd_body = "#D6EAF8") %>% 
  color( color="#FFFFFF", part="header" ) %>% 
  autofit() 
