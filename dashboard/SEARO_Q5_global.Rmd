---
title: ' '
pagetitle: 'SEARO Q5 global'
output:
  html_document:
    includes:
      in_header: SEARO_Q5_global_header.html
params:
  region: SEARO
---
...
other <- cbind( Region = data$Region, other) %>% 
  ...
  drop_na( Other ) %>%
  group_by( Region ) %>%
  summarize( Other = paste0(Other, collapse = ", ")) %>%
  ungroup()
...
qdf %>% 
  ...
  filter( Response == "Yes" ) %>%
  select( -c(Response,n,n_countries) ) %>%
  pivot_wider( names_from=Arbovirus, values_from=Percentage) %>%
