...
df <- df %>%
  mutate(Arbovirus = as.factor(new_cols[,1]), Testing = as.factor(new_cols[,2])) %>%
  mutate( Arbovirus = fct_relevel(Arbovirus, "Chikungunya", "Dengue", "Yellow fever", "Zika", "Other")) %>%
...
tab <- df %>%
    pivot_wider(names_from=Testing, values_from=Total, names_sort = TRUE) %>%
    regulartable %>%
    merge_v(j = ~Region) %>%
    valign(j = 1, valign = "top", part = "all") %>%
    hline(i=5*(1:4)) %>%
...
var_names <- c( "Chikungunya][Indirect",
                "Dengue][Indirect",
                "Yellow fever][Indirect",
                "Zika][Indirect",
                "Other][Indirect")
indirect <- data %>% select( all_of(var_names) )
colnames(indirect) <- gsub("][Indirect","",colnames(indirect), fixed=TRUE)

df <- cbind(Region = data$Region, indirect) %>% 
  pivot_longer(!Region, names_to="Arbovirus", values_to="Response") %>% 
  mutate( Response = factor( replace_na( as.character(Response), "No"))) %>%
  mutate( Response = fct_relevel(Response, "Yes", "No")) %>% 
  mutate( Arbovirus = fct_relevel(Arbovirus, "Chikungunya", "Dengue", "Yellow fever", "Zika", "Other")) %>% 
  group_by(Region, Arbovirus, Response) %>% 
  tally
tab <- df %>% 
    ...
    drop_na(Region) %>% 
    regulartable() %>% 
    set_caption("Indirect detection") %>%
    merge_v(j = ~Region) %>% 
    # valign(j = 1, valign = "top", part = "all") %>% 
    hline(i=5*(1:4)) %>% 
    fix_border_issues() %>%
...
var_names <- c( "Chikungunya][Direct",
                "Dengue][Direct",
                "Yellow fever][Direct",
                "Zika][Direct",
                "Other][Direct")
direct <- data %>% select( all_of(var_names) )
colnames(direct) <- gsub("][Direct","",colnames(direct), fixed=TRUE)

df <- cbind(Region = data$Region, direct) %>% 
  pivot_longer(!Region, names_to="Arbovirus", values_to="Response") %>% 
  mutate( Response = factor( replace_na( as.character(Response), "No"))) %>%
  mutate( Response = fct_relevel(Response, "Yes", "No")) %>% 
  mutate( Arbovirus = fct_relevel(Arbovirus, "Chikungunya", "Dengue", "Yellow fever", "Zika", "Other")) %>% 
...
var_names <- c( "Chikungunya][Both",
                "Dengue][Both",
                "Yellow fever][Both",
                "Zika][Both",
                "Other][Both")
both <- data %>% select( all_of(var_names) )
colnames(both) <- gsub("][Both","",colnames(both), fixed=TRUE)
df <- cbind( Country = data$SI01, both ) %>% 
  arrange( Country ) 
df %>% regulartable() %>% set_caption("Both direct and indirect detection")

var_names <- c( "Chikungunya][One",
                "Dengue][One",
                "Yellow fever][One",
                "Zika][One",
                "Other][One")
one_method <- data %>% select( all_of(var_names) )
colnames(one_method) <- gsub("][One","",colnames(one_method), fixed=TRUE)
df <- cbind( Country = data$SI01, one_method ) %>% 
  arrange( Country ) 
df %>% regulartable() %>% set_caption("Either direct or indirect detection (not both)")

var_names <- c( "Chikungunya][Neither",
                "Dengue][Neither",
                "Yellow fever][Neither",
                "Zika][Neither",
                "Other][Neither")
neither_method <- data %>% select( all_of(var_names) )
colnames(neither_method) <- gsub("][Neither","",colnames(neither_method), fixed=TRUE)
df <- cbind( Country = data$SI01, neither_method ) %>% 
  arrange( Country ) 
df %>% regulartable() %>% set_caption("Neither direct nor indirect detection")
