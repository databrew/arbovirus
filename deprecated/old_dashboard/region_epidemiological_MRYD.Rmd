...
#### Please provide the number of cases of locally acquired, mosquito-borne _Aedes_-borne arbovirus infections by case classification for 2020 and, if not available, for 2019. (Question 56) 
...
qdf <- cbind( Region = df$Region, responses) %>% 
  pivot_longer(!Region, names_to="Classification", values_to="Cases")
new_cols <- matrix( unlist( strsplit(qdf$Classification, "\\]\\[") ), ncol=2, byrow=T)
qdf <- qdf %>% 
  mutate(Arbovirus = new_cols[,1], Classification = new_cols[,2]) %>%
  group_by(Region, Arbovirus, Classification) %>% 
  drop_na( Region )
#  summarize(Total = sum(Cases, na.rm=T))

qdf %>%   
  pivot_wider(names_from=Classification, values_from=Cases) %>% 
